...
public partial class ServerData
{
    ...
    // This is the current save slot directory. MUST be set when we create/load
    // the server slot. We don't serialize this because it's possible the user
    // copies/moves/duplicates the original save folder, so it's better to just
    // set this when we load the world
    [MemoryPackIgnore]
    public string SaveDirectory { get; set; } = null!;

    // ID 0 will be reserved for wildcard, e.g. for entity ownership it means all players are owners
    public PlayerID PlayerIDCounter { get; set; } = 1;

    // Store username to player ID
    public Dictionary<string, PlayerID> PlayerIDs { get; set; } = [];
    ...
    public EntityID EntityIDCounter { get; set; }

    public SectorID SectorIDCounter { get; set; }

    // Store metadata about all sector names etc (key is sector ID)
    public Dictionary<SectorID, SectorMetadata> SectorMetadata { get; set; } = [];

    // Map entityID -> the sectorID it's located in
    public Dictionary<EntityID, SectorID> EntityDirectory { get; set; } = [];
    ...
    [MemoryPackIgnore]
    public Dictionary<SectorID, Sector> LoadedSectors { get; set; } = [];

    // Check if a sector exists
    public bool SectorExists(SectorID sectorID) => SectorMetadata.ContainsKey(sectorID);
